...
do
        ...
        ./wrap ${inputFiles[i]} ${lineWidths[i]} > temp
        ./wrap temp ${lineWidths[i]} | cmp ${outputFiles[i]}
        valgrind --leak-check=yes --error-exitcode=4 --log-file=temp ./wrap ${inputFiles[i]} ${lineWidths[i]} > temp
        ret=$?
        if [ $ret -eq 4 ]
        then
